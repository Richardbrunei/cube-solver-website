...
Your frontend and backend are **separate applications** that communicate via HTTP API calls:
...
│  URL: rubiks-frontend.onrender.com  │
└──────────────┬──────────────────────┘
               │
               │ HTTP Requests
               │ (fetch API calls)
...
│  URL: rubiks-backend.onrender.com   │
└─────────────────────────────────────┘
```

### How It Works:

1. **Frontend Configuration** (`scripts/config.js`):
   - Contains the backend URL: `API_BASE_URL`
   - Automatically switches between local and production

2. **API Calls** (JavaScript):
   - Frontend makes `fetch()` requests to backend
   - Example: `fetch('https://rubiks-backend.onrender.com/api/detect-colors')`

3. **CORS** (Backend):
   - Backend must enable CORS to allow frontend requests
...
```javascript
// 1. Frontend makes API call (scripts/camera-capture.js)
const response = await fetch(`${CONFIG.API_BASE_URL}/api/detect-colors`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ image: base64Image, face: 'front' })
});

// 2. Backend receives request (backend_api.py)
